...
class DenseEmbeddingWrapper(nn.Module):
    """
    Wrapper para exportação ONNX IGUAL ao pipeline antigo:
    - encoder HF
    - CLS pooling (token 0)
    - L2 normalization
    Saída: (batch_size, hidden_dim)
    """
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        cls = outputs.last_hidden_state[:, 0, :]          # <-- igual ao antigo
        return self.l2_normalize(cls)                      # <-- igual ao antigo
...
class BgeM3OnnxExporter:
    ...
    def build_dummy_inputs(
            ...
            dummy_texts = ["Dummy input for fixed ONNX export"] 
            encoded = tokenizer(
                dummy_texts,
                return_tensors="pt",
                padding="max_length",
            ...
            return {
                "input_ids": encoded["input_ids"],          # shape: [1, 1024]
                "attention_mask": encoded["attention_mask"],# shape: [1, 1024]
            }
    ...
    def export(self) -> str:
            ...
            print(f"Exportando BGE-M3 ONNX com shapes FIXOS: batch=1, seq_len={self.max_length}")                        
            ...
            with torch.no_grad():
                torch.onnx.export(
                    ...
                    dynamic_axes=None,  # ← ESSA é a chave: None = shapes 100% fixos
