...
### Para Reindexar os textos_treinamento , executar o comando abaixo no banco de dados:
# 1º Update textos_treinamento t set t.Indexado = False
...
####
class indexa_textos_treinamentoBll:
    ...
    def __init__(self, session: Session):        
        try:
            ...
            self.textos_final_collection_name = self.qdrant_utils.get_collection_name("final")
            self.qdrant_utils.create_collection(self.textos_final_collection_name)
    ...
    def _insert_text_list_qdrant(self, processed_data: list[dict]) -> list[dict]:
        try:
            ...
            result = self.qdrant_client.upsert(
                collection_name=self.textos_final_collection_name,
                points=points
            )
    ...
    def indexa_textos_treinamento(self) -> dict:
        ...
        self.qdrant_utils.force_reindex(self.textos_final_collection_name)
